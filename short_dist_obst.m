...
test_type = 'short_dist_obst'; % used to identify in output log
...
% options = optimoptions(@fmincon,'MaxIterations',4e4,'MaxFunctionEvaluations',8.2e5,'OutputFcn',@outputFcn_global,'OptimalityTolerance',1e-8,'Algorithm','active-set');
...
for kk = 1:size(obstacles,1)
    obst_points_temp = plot_obstacle(obstacles(kk,1),obstacles(kk,2:3));
%     ind_breaks = [ind_breaks,size(obst_points_temp,1)];
    if kk == 1
        obst_points_1 = obst_points_temp;
    else
        obst_points_2 = obst_points_temp;
    end
...
%         y_guess = flip(y_guess);
    case 4
        ...
        y_guess = -1*y_guess;
        y_guess = make_slope(y_guess,[3.2 5],[-1 4]);
...
function y_guess_out = make_slope(y_guess,x_bound,y_bound)
    global x_vals
    x_idx1 = x_bound(1);
    x_idx2 = x_bound(2);
    y_at1 = y_bound(1);
